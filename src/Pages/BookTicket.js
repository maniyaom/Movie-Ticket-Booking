...
const BookTicket = () => {
  ...
  useEffect(() => {
    const getMovieData = async () => {
      try {
        ...
        const initialSeats = movieData.theaterSeats.map((row) =>
          row.map((seat) => seat == 1)
        );
  ...
  const handleProceedToPay = () => {
    if (seatList.length == 0) alert("Please select atleast one seat");
    else if (userData.wallet >= seatList.length * ticketPrice) {
      setShowPopUp(true);
    } else alert("Insufficient Balance !! Please recharge your wallet.");
  };
  ...
  const handlePayment = () => {
    if (!movieDetails) {
      ...
      alert("Movie details are not available. Please try again.");
      return;
    }
    ...
    navigate(`/CheckOut/${userData.uid}`);
  };
  ...
  return (
    <>
      <div className="movie">
        <h2 style={{ fontSize: "28px", margin: "2px 20px" }}>
          {movieDetails.movieTitle}
        </h2>
        <p style={{ fontWeight: "500", margin: "6px 20px" }}>
          {movieDetails.theaterName} : {movieDetails.theaterAddress} |{" "}
          {movieDetails.movieReleaseDate} | {movieDetails.movieTiming}
        </p>
      </div>
      <div className="screen">
        <svg>
          <path
            d="M30 25 L25 30 L275 30 L270 25 Z"
            style={{ fill: "aliceblue", stroke: "lightblue" }}
          />
          <path
            d="M25 30 L35 55 L265 55 L275 30 Z"
            style={{ fill: "white", stroke: "lightblue" }}
          />
          <path
            d="M30 33 L37 52 L263 52 L270 33 Z"
      ...
      <div className={showPopUp ? "checkout-card" : "hide-element"}>
        ...
        <div className="text-key-value">
          ...
          <span className="values">
            {userData.wallet - seatList.length * ticketPrice} ₹
          </span>
        </div>
        <img
          src={loader_icon}
          className={isLoading ? "loader" : "hide-element"}
        />
        <button
          style={{
            fontSize: "15px",
            margin: "5px 0px 20px calc((100% - 10rem)/2)",
          }}
